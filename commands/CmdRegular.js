...
class CmdRotation extends CmdBase {
    ...
    defaultEmbed (_lang, _interaction) {
        const match = database.getListObject('Match', 'Regular')
    ...
    buildMessage (_lang, _rotation, _interaction) {
        const rotation = this.fetchRotation(_rotation).find(rot => rot.match === 'Regular')
        ...
        if (rotation && rotation.mode !== null) {
            ...
            embeds = this.defaultEmbed(_lang, _interaction)
        }
